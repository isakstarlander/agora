...
export const handler = async (event: FetchBodyEvent): Promise<FetchBodyResult> => {
  const { dok_id, doktyp, ingested } = event
  const url = `${BASE}/dokument/${dok_id}.text`
  let backoff = 250
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    if (statusCode === 404) {
      log.info({ dok_id }, 'fetch-body: 404 — body not available, skipping')
      await body.dump()
      return { dok_id, doktyp, ingested, skipped: true, s3_key: null }
    }
    ...
    if (RETRY_STATUSES.has(statusCode)) {
      ...
      log.warn({ statusCode, attempt, delay, dok_id }, 'fetch-body rate/error — retrying')
      await sleep(delay)
    ...
    const s3Key = `riks/document-text/${dok_id}.txt.gz`
    ...
    log.info({ dok_id, s3Key, bytes: bytes.length }, 'fetch-body written')
    return { dok_id, doktyp, ingested, skipped: false, s3_key: s3Key }
  }
